...
async def upload(message: Message, path: Path, del_path: bool = False, extra: str = ''):
    if path.name.upper().endswith((".MKV", ".MP4", ".WEBM")) and ('d' not in message.flags):
        await vid_upload(message, path, del_path, extra)
    elif path.name.upper().endswith((".MP3", ".FLAC", ".WAV", ".M4A")) and ('d' not in message.flags):
        await audio_upload(message, path, del_path, extra)
    elif path.name.upper().endswith((".JPG", ".JPEG", ".PNG", ".BMP")) and ('d' not in message.flags):
