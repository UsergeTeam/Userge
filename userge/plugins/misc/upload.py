...
async def upload(message: Message, path: Path, del_path: bool = False, extra: str = ''):
    if path.name.lower().endswith((".mkv", ".mp4", ".webm")) and ('d' not in message.flags):
        await vid_upload(message, path, del_path, extra)
    elif path.name.lower().endswith((".mp3", ".flac", ".wav", ".m4a")) and ('d' not in message.flags):
        await audio_upload(message, path, del_path, extra)
    elif path.name.lower().endswith((".jpg", ".jpeg", ".png", ".bmp")) and ('d' not in message.flags):
