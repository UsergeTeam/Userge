...
@userge.on_cmd('restart',
               about="Restarts the bot and reload all plugins")
async def restart_cmd_handler(client: userge, m: Message):
    ack_message = await m.reply(text=f"Restarting Userge Services",
                                reply_to_message_id=m.message_id)

    LOG.info(f"USERGE Services - Restart initiated")
    asyncio.create_task(restart(client, ack_message))
